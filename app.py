...
st.title("🏗️ Concrete Constitutive Parameter Prediction System")
...
FEATURE_MAP_ENG_TO_CHN = {
    ...
    'Unconfined Concrete Strength (Axial)': '非约束混凝土强度（轴心）',
...
if mode == "🖱️ Single Sample Manual Prediction":
    ...
    corrosion_stirrup = st.sidebar.number_input("Stirrup Corrosion Rate (%)", value=5.0, format="%g", step=0.1, min_value=0.0)
    corrosion_long = st.sidebar.number_input("Longitudinal Bar Corrosion Rate (%)", value=5.0, format="%g", step=0.1, min_value=0.0)
    fc = st.sidebar.number_input("Unconfined Concrete Strength (Axial) (MPa)", value=30.0, format="%g", step=0.1, min_value=0.0)
    ...
    fy_stirrup = st.sidebar.number_input("Stirrup Yield Strength (MPa)", value=400.0, format="%g", step=5.0, min_value=0.0)
    fy_long = st.sidebar.number_input("Longitudinal Bar Yield Strength (MPa)", value=400.0, format="%g", step=5.0, min_value=0.0)
    rho_v = st.sidebar.number_input("Volumetric Stirrup Ratio (%)", value=1.00, format="%g", step=0.1, min_value=0.0)
    rho_l = st.sidebar.number_input("Longitudinal Reinforcement Ratio (%)", value=1.00, format="%g", step=0.1, min_value=0.0)
    ...
    spacing_stirrup = st.sidebar.number_input("Stirrup Spacing (mm)", value=100.0, format="%g", step=5.0, min_value=0.0)
    ...
    if st.button("🚀 Start Prediction"):
        ...
        pred_stress = model_stress.predict(df_stress.rename(columns=FEATURE_MAP_ENG_TO_CHN))[0]
        pred_strain = model_strain.predict(df_strain.rename(columns=FEATURE_MAP_ENG_TO_CHN))[0]
        pred_r = model_r.predict(df_r.rename(columns=FEATURE_MAP_ENG_TO_CHN))[0]
        ...
        with res_col_left:
            ...
            st.metric(label="Peak Strain", value=f"{pred_strain/1000:.4f}") 
            ...
            if missing_cols:
                ...
                if st.button("🚀 Start Batch Prediction"):
                    with st.spinner("Calculating rapidly..."):
                        ...
                        # Rename English columns to Chinese under the hood before predicting
                        df_input['Predicted_Peak_Stress (MPa)'] = model_stress.predict(df_stress.rename(columns=FEATURE_MAP_ENG_TO_CHN))
                        df_input['Predicted_Peak_Strain'] = model_strain.predict(df_strain.rename(columns=FEATURE_MAP_ENG_TO_CHN)) 
                        df_input['Predicted_Deformation_Coefficient (r)'] = model_r.predict(df_r.rename(columns=FEATURE_MAP_ENG_TO_CHN))
